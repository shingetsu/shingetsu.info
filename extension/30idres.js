...
function pushdMaxRec() {
    ...
    var rec_n_size = ps[ps.length - 2].innerHTML;
...
function debug() {
    var ps = document.getElementsByTagName('p');
    var rec_n_size = ps[ps.length - 1].innerHTML;
    alert(rec_n_size);
    var maxrec = String(rec_n_size.match(/\(.*\/[0-9]+\//)).match(/[0-9]+/);
    alert(maxrec);
}
...
function addReplaceLink() {
    ...
    if (span.attachEvent) {
        span.attachEvent('onclick', res_id);
    } else if (span.addEventListener) {
        span.addEventListener('click', res_id, false);
    }
    p.appendChild(span);
}
...
function addDebugLink() {
    ...
    if (span.attachEvent) {
        span.attachEvent('onclick', debug);
    } else if (span.addEventListener) {
        span.addEventListener('click', debug, false);
    }
    p.appendChild(span);
}
...
function addonSubmit() {
    var form = document.getElementById('postarticle');
    if (form.attachEvent) {
        form.attachEvent('onsubmit', res_id);
    } else if (form.addEventListener){
        form.addEventListener('submit', res_id, false);
    }
}
